...
class PIIDetector(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        if found_pii:
            ...
            return FailResult(
                error_message="Phát hiện thông tin nhận dạng cá nhân (PII).",
...
class JSONFormatter(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        try:
            ...
            # TODO: Trả về PassResult với json.dumps(parsed, indent=2)
            return FailResult(
                error_message="JSON cần được chuẩn hóa định dạng.",
                fix_value=json.dumps(parsed, indent=2, ensure_ascii=False),
            )
        except json.JSONDecodeError as e:
            fallback = json.dumps(
                {"error": "invalid_json", "message": "Không thể sửa JSON đầu vào."},
                ensure_ascii=False,
            )
            return FailResult(
